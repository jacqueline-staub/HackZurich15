...
public class MyHttpConnection {
	...
	public static void loadGameState(String playerId){
		try {
			final String playerId2 = playerId;
			new AsyncTask<Void, Void, Void>() {
				...
				protected Void doInBackground(Void... params) {
					...
					try {
						...
						response = MyHttpConnection.doGet("http://phonewars.azurewebsites.net/api/getgamedata?playerId=" + playerId2);
					
					if(response != null && response.getInt("ErrorCode") !=0){
						Log.e("PhoneWars","GET ERROR: " + response.getString("ErrorMesssage"));
					}
					else if (response != null) {
						Game.state = response;
						Log.d("PhoneWars","Game state successfully loaded");
					}
					} catch (Exception e) {
						e.printStackTrace();
					}
			...
			Log.e("PhoneWars","Error: "+e.getMessage());
		}
	}
